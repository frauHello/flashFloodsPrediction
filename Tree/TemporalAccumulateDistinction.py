...
class TemporalAccumulateDistinction(Distinction):
        ...
        for graph in graphs.values():
            ...
            if all_attribs is not None:
                ...
                for attrib in all_attribs:
                    ...
                    len_values = len(vals)
                    max_extent = int(min(self.extent, current_flood_time))
                    #print("the extent for current graph is {0}".format(max_extent))
                    ...
                    for idx in range(current_flood_time-max_extent):
                        ...
                        if vals[idx] == no_data:
                            # print("val[idx] is equal to -1000.0")
                            continue

                        cumul = vals[idx]
                        for extent_step in range(1, max_extent):
                            j = idx + extent_step
                            if j > (current_flood_time):
                                break
                            if vals[j] == no_data: continue
                            cumul += vals[j]

                            if (cumul) >= split_value:
                                match = True
                                break
                        if match: break
    ...
    def getRandomDistinction(config, graphs):
        ...
        ####we are choosing an attribute randomly; qlthough in our grqph ze have only one attribute of array type
        ####but when making the split we must compute the cumul according to the flooding time of the new graph
        ###so what are the parameters of the split that we are going to take randomly.
        ...
        for graph in randomIter(graphs):
            # get all attributes of the correct type from this graph,but in our graphs we have only one rainfall array for
            # each graph
            ...
            for current_attrib in randomIter(all_attribs):

                real_attrib = current_attrib
                values = real_attrib.value
                ###values is the array of values of the current STattribute
            ...
            if real_attrib is None or len(values) - 1 < 3:
                print("This distinction is not valid")

            # pick the extent to calculate the cumulation over
            extent = np.random.randint(3, flood_time)
            ...
            while values[idx] == no_data:
                # print("val[idx] is equal to -1000.0")
                idx = np.random.randint(0, flood_time-extent)
            split_value = values[idx]
            for i in range(extent):
                if idx + i > flood_time - 1:
                    break
                if values[idx + i] == no_data:
                    continue
                split_value += values[idx + i]

            if split_value < 200:
                continue
            else:
                break
        #print("this is a temporal accumulation distinction with split value={0}, and extent={1}".format(split_value,extent))
        dist = TemporalAccumulateDistinction(attrib, extent, split_value)
    ...
    def __str__(self):
        s = 'TemporalAccumulateDistinction[attrib=%(attrib)r %%s, extent=%(extent)s, split=%(split_value)0.2F]' % self.__dict__
